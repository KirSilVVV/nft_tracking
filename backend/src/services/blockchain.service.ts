...
export class BlockchainService {
   ...
   * Note: fromBlock = 0 means "use recent blocks (last 45k blocks ~7 days)"
  ...
  private async _fetchTransferEvents(fromBlock: number = 0, toBlock?: number): Promise<Transaction[]> {
    try {
      ...
      // If fromBlock is 0, use recent blocks instead (last 45000 blocks ~7 days)
      // This avoids Alchemy API error: "fromBlock too old"
      if (fromBlock === 0) {
        fromBlock = Math.max(0, toBlock - 45000);
        logger.info(`📊 Block range: ${fromBlock} to ${toBlock} (last ~7 days, ~${toBlock - fromBlock} blocks)`);
      } else {
