...
export class ImageSearchService {
  ...
  private async getOrBuildHashIndex(): Promise<Map<number, string>> {
    ...
    // IMPORTANT: node-cache doesn't serialize Map properly, so we cache as plain object
    const cachedObj = getCacheService().get<Record<string, string>>(cacheKey);

    if (cachedObj) {
      const size = Object.keys(cachedObj).length;
      logger.info(`✅ Using cached hash index: ${size} NFTs`);
      return new Map(Object.entries(cachedObj).map(([k, v]) => [parseInt(k), v]));
    }
    ...
    try {
      ...
      logger.info(`✅ Loaded pre-built hash index from file: ${hashes.size} NFTs`);

      // Cache as plain object (node-cache doesn't handle Map properly)
      getCacheService().set(cacheKey, indexData, 86400);
      return hashes;
    } catch (error) {
      logger.warn('⚠️ Pre-built index not found, building sample index...');
    }
    ...
    // Cache as plain object for 24 hours
    const indexDataForCache = Object.fromEntries(hashes.entries());
    getCacheService().set(cacheKey, indexDataForCache, 86400);
    ...
    try {
      ...
      await fs.writeFile(indexPath, JSON.stringify(indexDataForCache, null, 2));
